%% main_Hyperbolic1D_LaxFriedrichs.m
% 
...
%     u(x,0) = u0(x)
%
% using the Lax-Friedrichs scheme (lam = dt/dx):
%
...
for t = t0:dt:tf
    ...
    uf = u0;  % t_{n+1}
    % compute by scheme
    uf(i+1:end-1) = 0.5*( u0(i+2:end) + u0(i:end-2) ) ...
                    - 0.5*a*lam*( u0(i+2:end) - u0(i:end-2) );  
    % show
